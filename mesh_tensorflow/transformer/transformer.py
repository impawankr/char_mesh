...
class Unitransformer(object):
    ...
    # ----------------
        ...
        if output_vocab_size:
            ...
            if autoregressive:
                raise ValueError(
                    "autoregressive Transformer needs output vocabulary")
    ...
    def _call_internal(self, context, inputs, targets=None):
        ...
        if self.ensemble_dim and self.ensemble_dim not in inputs.shape.dims:
            ...
            inputs = mtf.broadcast(
                inputs, [self.ensemble_dim] + inputs.shape.dims)
            if targets:
        ...
        inputs = mtf.dropout(inputs, context.train,
                             rate=self.token_dropout_rate)
    ...
    def call_simple(self,
        ...
        if self.input_full_attention:
            ...
            full_attention_region = mtf.logical_or(
                ...
                mtf.shift(full_attention_region, offset=1,
    ...
    def sample_autoregressive(self,
        ...
        def body_fn(position, ids, *states):
            ...
            with tf.variable_scope(self.name, reuse=True):
                logits = self._call_internal(
                    context_incremental, inputs_this_step)
                if never_end:
                    logits += mtf.one_hot(
                        mtf.constant(logits.mesh, stop_at_token,
                                     dtype=tf.int32),
...
class Bitransformer(object):
    ...
    def decode(self,
        ...
        if beam_size == 1:
            ...
            partial_sequences = mtf.zeros(
                inputs.mesh, ids_shape, dtype=tf.int32)
            ...
            if isinstance(cls, (list, tuple)):
                for x in cls:
                    if isinstance(x, str):
                        ...
                        cls = x
            if block_scope:
                name = "block_{:03d}/{}".format(block,
                                                name or "layer_{:03d}".format(n))
            else:
